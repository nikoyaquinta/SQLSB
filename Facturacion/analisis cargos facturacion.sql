...
from vale_asis_cab cab
	...
	--Anulación de Actividad ( Llamada desde nvuo_actividad_det.uof_anular_apunte_en_va )+ modulo confcita = SE ANULO LA CITA ASOCIADA A ESTA PRESTACION
	  
	--Anulación de Actividad ( Llamada desde nvuo_actividad_det.uof_anular_apunte_en_va )+ modulo bloque_quirurgico = SE ANULO ESTA PRESTACION DESDE LA FICHA

	--Anulación de Actividad ( Llamada desde nvuo_actividad_det.uof_anular_apunte_en_va ) + modulo ingresos = SE HACE UN CAMBIO DE PAGADOR DEL ENCUENTRO Y SE PASAN LOS CARGOS AL NUEVO FINANCIADOR

	--Cambio de Pagador de Vales Pendientes ( llamada desde nvuo_encuentro.uof_cambiar_pagador_vale, que a su vez se llama desde uo_control_facturas de Facturación o desde uo_prest_pago de Caja Convenios  ( consultar campo modulo_anul_vale )  ) + modulo facturacion = SE REALIZO CAMBIO DE PAGADOR DE LA PRESTACION